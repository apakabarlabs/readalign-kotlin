 ...
 *
 * A recogniser answers in tokens, not in words: `be`, `aut`, `y's`. Where one word ends and
 * the next begins is the recogniser's convention, not the caller's, and every side that
 * reads the same model has to read that convention the same way — otherwise one returns
 * `beauty's` where another returns `beautys` and a third splits the word in two, and the
 * three cannot be held against each other however alike they heard the sound.
 *
 ...
 * `increase`, and the comma is the model's punctuation rather than anything it heard. Marks
 * inside a word stay, because `beauty's` and `self-substantial` are words and `beautys` is
 * not.
 */
fun spoken(tokens: List<RecognizedWord>): List<RecognizedWord> {
    ...
    for (token in tokens) {
        for (character in token.text.replace(UNHEARD, "")) {
            if (character in OPENS_A_WORD) {
                if (text.isNotEmpty()) {
                    words.add(RecognizedWord(text.toString(), start, end))
                    text = StringBuilder()
                }
                continue
            }
    ...
    if (text.isNotEmpty()) words.add(RecognizedWord(text.toString(), start, end))
    return words.mapNotNull(::trimmed)
}

private fun trimmed(word: RecognizedWord): RecognizedWord? {
    val text = word.text.trim { !it.isLetterOrDigit() }
    return if (text.isEmpty()) null else RecognizedWord(text, word.start, word.end)
}
